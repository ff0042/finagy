...
export default function Header() {
  ...
  // Dynamically calculate live market value from streaming prices
  let livePosValue = 0;
  portfolio.positions.forEach((p: any) => {
    const cp = prices[p.ticker]?.price || p.current_price || p.avg_cost;
    livePosValue += cp * p.quantity;
  });
  const liveTotalValue = portfolio.cash_balance + livePosValue;

  return (
    <header className="flex items-center justify-between p-4 bg-card rounded-lg mb-4">
      ...
      <div className="flex gap-8">
        ...
        <div className="text-right">
          ...
          <p className="text-xl font-mono text-white">${portfolio.cash_balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
        </div>
      </div>
    </header>
  );
